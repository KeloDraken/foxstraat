...
from foxstraat.core.blog.forms import CreateBlogPostForm
from foxstraat.core.blog.models import Blog
...
def add_blog_post(request):
    if not request.user.is_blogger:
        messages.error(
            request,
            "You're not a blogger yet. Go to Edit profile to add feature to your account",
        )
        ...
        if request.method == "POST":
            ...
            if not captcha_data == "":
                if post_form.is_valid():
                    ...
                    messages.error(request, "An error occured. Couldn't save your post")
            else:
        ...
        return render(request, "views/blog/add_blog_post.html", context)
