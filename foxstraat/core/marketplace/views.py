...
from foxstraat.utils.helpers import forbidden_attributes, object_id_generator
    ...
    if request.method == "POST":
        try:
            ...
            if not len(name) <= 0 and not name == None:
                ...
                messages.error(request, "Couldn't add listing because of a name error")
        ...
        try:
            ...
            if not len(description) <= 0 and not description == None:
                ...
                messages.error(
                    request, "Couldn't add listing because of a description error"
                )
                return redirect("marketplace:add-listing")
        except:
            messages.error(
        ...
        try:
            ...
            if not len(price) <= 0 and not price == None:
                try:
                    ...
                    messages.error(
                        request, "Couldn't add listing because of a price error"
                    )
                    return redirect("marketplace:add-listing")
            else:
                messages.error(request, "Couldn't add listing because of a price error")
                return redirect("marketplace:add-listing")
        except:
            messages.error(request, "Couldn't add listing because of a price error")
            return redirect("marketplace:add-listing")
        ...
        try:
            ...
            if not template == None and not len(template) <= 0:
                ...
                messages.error(request, "Couldn't add listing because of a CSS error")
                return redirect("marketplace:add-listing")
        ...
        if request.FILES.get("screenshot"):
            ...
            messages.error(request, "Couldn't add listing because of a screeshot error")
            return redirect("marketplace:add-listing")
...
def manage_listings(request):
    ...
    if not posts:
        messages.error(
            request, "You don't have any marketplace listings. Create your listing"
        )
        return redirect("marketplace:add-listing")
    else:
        context = {"posts": posts}

        return render(request, "views/marketplace/manage_listings.html", context)
...
def view_listing(request, listing_id):
    ...
    context = {"post": post}
    return render(request, "views/marketplace/view_listing.html", context)
...
def buy_template(request, listing_id):
    if request.method == "POST":
        ...
        return render(request, "views/marketplace/buy_template.html", context)
